...
def web_test_label(wav_file_path):
    K.clear_session()
    ...
    for item in preds1:
        if item == 0:
            ...
            return "UNABLE TO IDENTIFY EMOTION"
...
def main():
    ...
    ax1.set_title("An example of Raw wave of " + filename)
    ...
    ax2.set_title("An example of Spectrogram of " + filename)
    ...
    plt.title("Mel power spectrogram of trimmed audio")
    ...
    # Original Sound
    librosa.output.write_wav("Original_Audio.wav", samples, sample_rate)

    # Silence trimmed Sound by librosa.effects.trim()
    librosa.output.write_wav("Trimmed_Audio.wav", aa, sample_rate)
    ...
    # Examples of actors with id: 21, 22, 23, 24
    ...
    plot_time_series(X)
    librosa.output.write_wav("Kaiser_fast_Audio.wav", samples, sample_rate)
    ...
    '''
    y_train = to_categorical(y_train.factorize()[0])
    y_test = to_categorical(y_test.factorize()[0])
    '''
    y_train = keras.utils.to_categorical(y_train.factorize()[0], num_classes=5)
    y_test = keras.utils.to_categorical(y_test.factorize()[0], num_classes=5)
    ...
    class_names = ['male_calm', 'male_happy', 'male_sad', 'male_angry', 'male_fearful']
